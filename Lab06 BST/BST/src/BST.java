...
public class BST {
	...
	public int findNextData(int n) {
		...
		if (root == null)
			return n;
		BSTNode temp = root;
		TreeIterator itr = findMax(temp.right);
		TreeIterator itr2 = findMin(temp.left);
		int min = itr2.currentNode.data;
		int max = itr.currentNode.data;
		if (n >= max)
			return n;
		if (n < min)
			return min;
		...
		for (int i = 0; i < data2.size(); i++) {
			int diff = data2.get(i) - n;
			if (diff > 0 && diff < next - n)
				next = data2.get(i);
		}
		return next;
	}
