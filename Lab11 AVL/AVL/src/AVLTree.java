public class AVLTree {
	...
	public void makeAVL() throws Exception {
		AVLNode min = ((AVLTreeIterator) this.findMin()).currentNode;
		AVLTreeIterator avlItr = new AVLTreeIterator(min);
		AVLTree avlTree = new AVLTree();
		while (avlItr.hasNext()) {
			avlTree.insert(avlItr.currentNode.data);
			avlItr.next();
		}
		avlTree.insert(avlItr.currentNode.data);
	...
	public boolean isAVL() {
		if (this.root == null)
			return true;
		AVLTree subtreeLeft = new AVLTree();
		subtreeLeft.root = this.root.left;
		AVLTree subtreeRight = new AVLTree();
		subtreeRight.root = this.root.right;
		return Math.abs(AVLNode.tiltDegree(this.root)) <= 1 && subtreeLeft.isAVL() && subtreeRight.isAVL();
	}
	...
	public static boolean same(AVLTree t1, AVLTree t2) {
		...
		if (t1.root.data != t2.root.data)
			return false;
		AVLTree t1SubtreeLeft = new AVLTree();
		AVLTree t1SubtreeRight = new AVLTree();
		AVLTree t2SubtreeLeft = new AVLTree();
		AVLTree t2SubtreeRight = new AVLTree();
		t1SubtreeLeft.root = t1.root.left;
		t1SubtreeRight.root = t1.root.right;
		t2SubtreeLeft.root = t2.root.left;
		t2SubtreeRight.root = t2.root.right;
		return same(t1SubtreeLeft, t2SubtreeLeft) && same(t1SubtreeRight, t2SubtreeRight);
