...
public class Sorting {
  ...
  public static void main(String[] args) throws Exception {
    int repeat = 2; // repeat each experiment a couple of times and keep the best result (lowest
                    // time)

    for (int initialOrder = 0; initialOrder <= 2; initialOrder++) { // 0 = sorted, 1 = random, 2 = reversed
      System.out.printf("\nInitial state = %d\n", initialOrder);
      System.out.printf("size \t time(ms) \n");
      // for (int n = 10000; n <= 160000; n *= 2) { // bubble, selection, insertion
      // sort
      for (int n = 1000000; n <= 64000000; n *= 2) { // shell, heap, merge, quick
        // sort
        int[] d = new int[n];
        switch (initialOrder) {
        case 0:
  ...
  private static void merge(int[] d, int left, int mid, int right, int[] t) {
    int i = left, j = mid + 1;
    int k = left;
    while (i <= mid && j <= right) {
      t[k++] = d[i] < d[j] ? d[i++] : d[j++];
